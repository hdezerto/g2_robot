...
class MapBuilder:
    ...
    def initialize_map(self, min_x, max_x, min_y, max_y):
        ...
        # Initialize maps: Occupancy is unknown (205), confidence starts at 0
        self.map = 205 * np.ones((self.size_y, self.size_x), dtype=np.uint8)
        self.confidence_map = np.zeros((self.size_y, self.size_x), dtype=np.uint8)
    ...
    def add_scan(self, scan_data):
        ...
        for x, y in scan_data:
            ...
            if 0 <= mx < self.size_x and 0 <= my < self.size_y:
                ...
                self.map[my, mx] = int(205 - (self.confidence_map[my, mx] * 2.05))
